    ...
    for (const t of tokens) {
        if (typeof t === 'number') {
            ...
            while (opstk.length) {
                ...
                const o1info = OPS[o1];
                const o2info = OPS[o2];
                const cond = (o1info.assoc === 'L' && o1info.p <= o2info.p) ||
...
function extractNumbers(tokens) {
    ...
    for (const t of tokens) {
        if (typeof t === 'number') {
            if (t >= 1 && t <= 13 && Number.isInteger(t))
                ns.push(t);
        }
    }
    return ns;
}
export function validateAndEvalExpression(numbers, expr) {
    const res = {
        ok: false,
        errors: [],
        value: undefined,
    ...
    try {
        ...
        // 数字使用需要与题面四数完全一致（多重集）
        res.usesAllNumbers = multisetEqual([...numbers].sort((a, b) => a - b), [...used].sort((a, b) => a - b));
...
// 是否等于24（带容差）
export function isTwentyFour(v) {
